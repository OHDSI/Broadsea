...
PGPASSWORD=$(</run/secrets/PHOEBE_PG_PASSWORD) psql -v vocab_schema=$PHOEBE_PG_SCHEMA -h $PHOEBE_PG_HOST -U $PHOEBE_PG_USER -d $PHOEBE_PG_DATABASE -a -f concept_recommended.sql

echo 'Now loading: concept_recommended'
PGPASSWORD=$(</run/secrets/PHOEBE_PG_PASSWORD) psql -h $PHOEBE_PG_HOST -d $PHOEBE_PG_DATABASE -U $PHOEBE_PG_USER \
